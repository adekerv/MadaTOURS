...
const trip = {
  ...
  name: 'Saturday',
  date: '2026-09-26',
  notes: 'Water and picnic',
  stops: [
    { placeId: 2, minutes: 60 },
    { placeId: 3, minutes: 90 },
  ],
  updatedAt: '2026-09-23T10:00:00.000Z',
};
test('day trips preserve ordering, validate dates, and reject malformed or oversized saved data', () => {
  assert.deepEqual(
    moveStop(trip, 0, 1).stops.map((s) => s.placeId),
    [3, 2],
  );
  assert.equal(moveStop(trip, 0, -1), trip);
  ...
  assert.equal(
    tripSchema.safeParse({ ...trip, stops: [trip.stops[0], trip.stops[0]] }).success,
    false,
  );
...
test('French translations preserve all interpolation placeholders', () => {
  for (const [en, fr] of Object.entries(french))
    assert.deepEqual(
      ...
      [...fr.matchAll(/\{(\w+)\}/g)].map((m) => m[1]).sort(),
      en,
    );
