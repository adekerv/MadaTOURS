...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const mappedPlaces = placesData.filter((p: any) => revList.some((r: any) => r.place_id === p.id));
      return res.status(200).json(mappedPlaces);
    } catch (err: any) {
      return res.status(500).json({ error: 'Failed to query user_revisits' });
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Failed to insert revisit' });
    }
  }
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Failed to delete revisit' });
    }
  }
